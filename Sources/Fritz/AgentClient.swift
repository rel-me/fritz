...
public struct AgentFailure: LocalizedError {
    public let message: String
    public init(message: String) { self.message = message }
    public var errorDescription: String? { message }
}

public struct AgentEvent: Decodable, Sendable {
...
@MainActor @Observable public final class AgentClient {
    ...
    /// The host owns executable selection and environment; never put credentials in either.
    public init(
        executableURL: URL?,
    ...
    public func start() {
        ...
        guard let executableURL, FileManager.default.isExecutableFile(atPath: executableURL.path) else {
            startupError = "The bundled agent is missing. Build the complete app before starting it."
            return
        }
    ...
    public func stream(method: String, params: [String: Any] = [:], id: String = UUID().uuidString) -> AsyncThrowingStream<Data, Error> {
    ...
    public func request<T: Decodable>(_ method: String, params: [String: Any] = [:], as: T.Type = T.self) async throws -> T {
