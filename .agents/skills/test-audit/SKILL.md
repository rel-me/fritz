...
- Rust agent, CLI, and state (`src/`, `crates/`, `tests/*.rs`);
- Swift libraries (`Sources/`, `tests/Fritz*Tests`);
- the app module (`app/Sources`, `app/Tests`);
- Python integration harnesses, scripts, and tooling (`tests/*.py`, `scripts/`);
...
3. Run `make check`, then `git diff --check`.
4. Run `make test` before reporting completion; add `make build` and the
   affected workflow when runtime, agent, or packaging behavior changed.
...
Commit, push, open a PR, or land only when authorized, following the commit and
PR rules in `AGENTS.md`. Land one coherent PR at a time; after landing, refresh
