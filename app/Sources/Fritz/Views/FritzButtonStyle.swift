...
struct FritzButtonStyle: PrimitiveButtonStyle {
    ...
    init(_ context: Context = .content, size: ControlSize? = nil, shape: ButtonBorderShape = .capsule) {
    ...
    func makeBody(configuration: Configuration) -> some View {
        ...
        case .floatingPrimary:
            if #available(macOS 26.0, *) {
                button(configuration).buttonStyle(.glassProminent).buttonBorderShape(shape).controlSize(controlSize)
            } else {
                button(configuration).buttonStyle(.borderedProminent).controlSize(controlSize)
            }
        case .inline:
...
extension View {
    ...
    func fritzButtonSize(_ size: ControlSize) -> some View {
        environment(\.fritzButtonControlSize, size)
    }
}

struct FritzGlassControlGroup: ViewModifier {
    @ViewBuilder
...
struct FritzPanelIconControl: ViewModifier {
