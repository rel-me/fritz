...
The source version is `0.1.1` in `Cargo.toml` and `app/project.yml`, with Sparkle build number `2`. Release builds use those values by default. Ordinary local builds have no update feed; distribution targets embed Fritz's separate Sparkle public key and `https://fritz.rel.me/appcast.xml`. The private key remains in the macOS Keychain under the `fritz` Sparkle account.
