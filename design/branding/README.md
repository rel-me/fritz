...
After editing `FritzRaven.svg` or `00-Background.svg`, run `python3 design/branding/export.py` from the repository root (requires `rsvg-convert` from librsvg). It generates `01-Raven.svg` with the icon's placement, copies both icon layers into the Icon Composer document, and exports the light/dark SVG logos and vector PDF chat marks. It also writes the home page's raven mask, adaptive favicon, touch icon, and social card to `website/public/`. The wordmarks use Avenir Next with Avenir and sans-serif fallbacks.
